...
export const Template = (args) => {
  return (
    <div style={{ height: "600px", width: "800px" }}>
      <Chart {...args}>
        <Chart.LineSeries data={[{ time: "2018-12-22", value: 32.51 }]} />
      </Chart>
    </div>
  );
};
...
<Canvas isColumn>
  <Story name="Line chart">{Template.bind({})}</Story>
  <Story
  ...
  <Story
    name="Bar chart"
    parameters={getFigmaParameters(
      "https://www.figma.com/file/YjE625ScDMf2ILjWB1sTMc/System?type=design&node-id=546-5496&mode=design&t=7dumrRXwuL31wuyv-0"
    )}
  >
    {Template.bind({})}
  </Story>
