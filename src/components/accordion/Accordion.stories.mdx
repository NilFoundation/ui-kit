...
export const Template = ({isIcon, isDescription, ...args}) => {
    ...
    return (
      <div style={{maxWidth: "530px"}}>
        <Accordion {...args}>
          <Panel title="Panel 1" description={isDescription && description} icon={isIcon && <Filter />}>Content 1</Panel>
          <Panel title="Panel 2" description={isDescription && description} icon={isIcon && <Filter />}>Content 2</Panel>
          <Panel title="Panel 3" description={isDescription && description} icon={isIcon && <Filter />}>Content 3</Panel>
        </Accordion>
      </div>
    );
}
